...
output$varSelection_stn <- renderUI({
  ...
  radioButtons(inputId = "compare_var", label = "Select one variable: ", choices = var_subset, inline = FALSE, selected = "Temp")
})
...
stn_compare_data_query <- reactive({
  ...
  withProgress(message = 'Requesting Data... ', value = 1, {
    ...
    query <- lapply(input$station_site, function(x) paste0("SELECT DateTime, WatYr, ", input$compare_var, ", '", x,"' as station FROM raw_Ttherm_test_", x," where WatYr = ", input$station_year)) %>%
